...
class CustomTestItem extends Item {
    CustomTestItem(Item.Settings settings) {
        super(settings)
    }
...
class CustomTestBlock extends Block {
    CustomTestBlock() {
    ...
    @Override
    ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
...
// Register the custom block and item
